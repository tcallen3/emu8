...
static ReportInvalidAccess(Word addr)
{
...
Memory8::Memory8(const std::size_t memBase) :
	memLow_(memBase)
{
	memory_.fill(0x0);
}
...
void Memory8::SetSequence(Word addr, Word size, 
...
void Memory8::CoreDump(const std::string& coreFile) const
{
	...
	if (!coreStream.good()) {
		throw std::runtime_error("Could not open corefile: " + 
			coreFile);
	}
